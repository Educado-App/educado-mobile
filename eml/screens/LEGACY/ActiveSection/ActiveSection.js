...
const getComponents = async (components) => {
  ...
  const res = await axios.post(
    'http://colibri.somethingnew.dk/api/component/getallcomponents',
    obj
  )
  return res.data
}
...
class ActiveSection extends Component {
  ...
  async componentDidMount() {
    const componentsTemp = await getComponents(
      this.props.navigation.state.params.section.components
    )
    this.setState({
      ...this.state,
